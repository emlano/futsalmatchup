...
describe("GET /users", () => {
    ...
    test("should disallow when token is expired", async () => {
        ...
        const token = jwt.sign(testdata, process.env.ACCESS_TOKEN_SECRET, { expiresIn: -1 })
...
describe("GET /users/other", () => {
    test("Should return an error when arguments were not provided", async () => {
        ...
        const res = await request(app).get("/users/other").set("Authorization", `Bearer ${token}`).send([])
    ...
    test("should return an error when username or id is missing", async () => {
        ...
        expect(res.status).toBe(400)
        expect(res.body).toEqual({ error: "requested users id or username not given" })
    })
...
describe("POST /users/signup", () => {
    test("Should return an error when arguments were not provided", async () => {
        ...
        const res = await request(app).post("/users/signup").set("Authorization", `Bearer ${token}`).send([])
    ...
    test("should return an error if username or password was not given", async () => {
        ...
        const res2 = await request(app).post("/users/signup").send([{ "username": "name" }])
        const res3 = await request(app).post("/users/signup").send([{ "password": "pass" }])
    ...
    test("should return a token when inputs are valid", async () => {
        const testdata = [{ "user_id": 1, "username": "user", "password": "password" }]
        const token = jwt.sign(testdata[0], process.env.ACCESS_TOKEN_SECRET, { expiresIn: 60 * 60  })
...
describe("POST /users/login", () => {
    test("Should return an error when arguments were not provided", async () => {
        ...
        const res = await request(app).post("/users/login").set("Authorization", `Bearer ${token}`).send([])
    ...
    test("should return token when provided info is correct", async () => {
        ...
        const token = jwt.sign(resultdata[0], process.env.ACCESS_TOKEN_SECRET, { expiresIn: 60 * 60 })
...
describe("PUT /users/", () => {
    ...
    test("Should modify data when info was provided", async () => {
        ...
        const res = await request(app).put("/users/").set("Authorization", `Bearer ${token}`).send([{ "player_city": "New York" }])

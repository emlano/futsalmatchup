...
router.post('/login', async (req, res) => {
    try {
        ...
        if (await bcrypt.compare(candidate.password, user.password) && await bcrypt.compare(candidate.username, hashedUsername)) {
            const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET, { expiresIn: 60 * 60 })
            res.send({ accessToken: accessToken })
...
function authenticateToken(req, res, next) {
    ...
    jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user) => {
        if (err) return res.status(403).send({ error: "malformed or expired token" })
